...
calculate (struct calculation_arguments* arguments, struct calculation_results *results, struct options* options)
{
	...
	double residArray[N][N];
	for(int i = 0; i < N; i++)
	{
		for(int j = 0; j < N; j++)
		{
			residArray[i][j] = TWO_PI_SQUARE * sin((double)(j) * PI * arguments->h) * sin((double)(i) * PI * arguments->h) * arguments->h * arguments->h;
		}
	}

	double *ibefore = Matrix[m2][0];
	double *inow = Matrix[m2][1];
	double *inext;
	...
	while (options->term_iteration > 0)
	{
		...
		for (i = 1; i < N; i++)
		{
			...
			for (j = 1; j < N; j++)
			{
				star = -ibefore[j] - inow[j-1] - inow[j+1] - inext[j] + 4.0 * inow[j];
				...
				{
					residuum = (residArray[i][j] - star) / 4.0;
				}
				Matrix[m1][i][j] = inow[j] + residuum;
		...
		if (options->termination == TERM_PREC && maxresiduum < options->term_precision)
		{
