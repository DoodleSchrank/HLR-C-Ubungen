...
int main(void)
{
	...
	MPI_Type_struct(2, {1, 1} {0, 1}, {MPI_CHAR, MPI_LONG}, &datastruct);
	MPI_Type_commit(&datastruct);
	
	if( == 0) master(numThreads);
	else slave();
...
void master(int numThreads)
{
	MPI_Datatype datastruct;
	for(int i = 1; i < numThreads; i++)
	{
		MPI_Recv(&datastruct, 1, MPI_Type_struct, i, void , void , void);
